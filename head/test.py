...
parser.add_argument('-m', '--trained_model', default='/project/train/models/final/RetinaFace.pth',
...
if __name__ == '__main__':
    ...
    args.cpu = False if USE_GPU else True
    ...
    # testing begin
    for i in range(1):
        img_path = '/project/train/processed_data'
...
#        image_path = "test.jpg" #"./curve/test.jpg"
#        img_raw = cv2.imread(image_path, cv2.IMREAD_COLOR)
        ...
        scores_all = conf.squeeze(0).data.cpu().numpy()
        ...
        cls_idx = cls_idx[:args.keep_top_k]

        # show image
        if args.save_image:
            for (b,c) in zip(dets, cls_idx):
                if b[4] < args.vis_thres:
                    continue
                text = "{}:{:.4f}".format(cfg['classes'][c],b[4])
                print(text)
                b = list(map(int, b))
